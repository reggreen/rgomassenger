...
// This ensures real-time behavior and persistent data even in preview mode without API keys!
  ...
  if (!localStorage.getItem('rg_tasks')) {
    setLocalData('rg_tasks', [
      { 
        ...
        due_date: new Date(Date.now() + 3600000 * 3).toISOString(), // 3 hours from now
      ...
      { 
        ...
        due_date: new Date(Date.now() + 3600000 * 24).toISOString(), // 24 hours from now
        email: 'redgreenonline2023@gmail.com',
        channels: 'In-app',
        priority: 'মাঝারি',
        status: 'Pending',
...
// Custom EventEmitter for in-tab real-time sync
  ...
  channel: (channelName) => {
    return {
      on: (eventType, filter, callback) => {
        const handleMsg = (e) => {
          // Verify if it is relevant to the table filter
        ...
        // Listen to in-tab updates
        localListeners.add(handleMsg);

        return {
          subscribe: () => {
            // Return an object that has an unsubscribe method
            return {
              unsubscribe: () => {
