    ...
    supChannel.on = (eventType, filter, callback) => {
      if (eventType === 'postgres_changes') {
        try {
          ...
          console.warn('Postgres changes subscription warning:', e);
        }
      }
      ...
      return {
        subscribe: () => {
          try {
            supChannel.subscribe();
          } catch (e) {
            console.warn('Channel subscribe warning:', e);
          }
          return {
            unsubscribe: () => {
              try {
              ...
              localTypingListeners.delete(handleTypingMessage);
              if (typingChannel) {
                typingChannel.removeEventListener('message', handleTypingMessage);
              }
            }
          };
        }
  ...
  channel: (channelName) => {
    return {
      on: (eventType, filter, callback) => {
        const handleMsg = (e) => {
          if (!filter || !filter.table || e.table === filter.table || e.event === 'TYPING') {
            callback(e);
          }
        };
        ...
        return {
          subscribe: () => {
            return {
              unsubscribe: () => {
                localTypingListeners.delete(handleMsg);
                if (typingChannel) {
                  typingChannel.removeEventListener('message', handleMsg);
                }
              }
            };
          }
