...
if (typeof window !== 'undefined') {
  ...
  try {
    ...
    if (!isCleaned) {
      ...
      // Purge dummy mock users and sanitize unique IDs from rg_all_users
      ...
      if (storedUsers) {
        try {
          ...
          const realUsers = parsed
            .filter(u => 
              u.email?.toLowerCase() === 'redgreenonline1013@gmail.com' || 
            ...
            .map((u, idx) => {
              ...
              let id = u.id;
              if (clean === 'redgreenonline2023@gmail.com' && (!id || id === 'usr_admin_01')) {
                id = 'usr_admin_02';
              } else if (clean === 'redgreenonline1013@gmail.com') {
                id = 'usr_admin_01';
              }
              if (!id || seenIds.has(id)) {
                id = clean ? `usr_${clean.replace(/[^a-zA-Z0-9]/g, '_')}` : `usr_clean_${idx}`;
              }
              seenIds.add(id);
              return { ...u, id };
            });
      ...
      const storedProfiles = localStorage.getItem('rg_profiles');
      if (storedProfiles) {
        try {
          const parsedP = JSON.parse(storedProfiles);
          const seenP = new Set();
          const cleanP = parsedP.map((p, idx) => {
            ...
            if (!id || seenP.has(id)) {
              id = clean ? `usr_${clean.replace(/[^a-zA-Z0-9]/g, '_')}` : `usr_prof_${idx}`;
            }
            seenP.add(id);
            return { ...p, id };
          });
          localStorage.setItem('rg_profiles', JSON.stringify(cleanP));
        } catch (e) {}
      }
